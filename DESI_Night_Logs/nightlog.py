...
time_sunrise,time_moonrise,time_moonset,illumination,weather_conditions):
    ...
    def add_problem_os(self,time,problem):
        """
            Operations Scientist adds details on a problem encountered.
        """

        the_path=self.os_pb_dir+self.get_timestamp(time)
        file=self.new_entry_or_replace(the_path)
        file.write("- "+time[0:2]+":"+time[2:4]+" := "+problem+"\n")
        file.close()
    ...
    def checklist_os(self,time):

        the_path = self.os_cl
        if os.path.exists(the_path):
            file = open(the_path,'a')
            file.write("OS checklist completed at "+time[0:2]+":"+time[2:4])
            file.close()
        else:
            file = open(the_path,'a')
            file.write(", "+time[0:2]+":"+time[2:4])
            file.close()


    def add_problem_dqs(self,time,problem):
        """
            Data Quality Scientist adds details on a problem encountered.
        """
    ...
    def checklist_dqs(self,time):

        the_path = self.dqs_cl
        if os.path.exists(the_path):
            file = open(the_path,'a')
            file.write("DQS checklist completed at "+time[0:2]+":"+time[2:4])
            file.close()
        else:
            file = open(the_path,'a')
            file.write(", "+time[0:2]+":"+time[2:4])
    ...
    def finish_the_night(self):
        ...
        file_nl.write("\n")
        file_nl.write("\n")
        compile_entries(self.os_pb_dir,file_nl)
        file_nl.write("h5. encountered by the DQS\n")
        file_nl.write("\n")
        file_nl.write("\n")
        compile_entries(self.dqs_pb_dir,file_nl)
        ...
        file_nl.write("\n")
        file_nl.write("\n")
        compile_entries(self.os_startcal_dir,file_nl)
        compile_entries(self.os_obs_dir,file_nl)
        file_nl.write("h3. Details on the night progress from the DQS (local time)\n")
        file_nl.write("\n")
        file_nl.write("\n")
        compile_entries(self.dqs_exp_dir,file_nl)
        file_nl.close()
